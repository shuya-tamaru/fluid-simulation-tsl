...
export function computePressureForcePass(
  ...
  return Fn(() => {
    ...
    If(i.lessThan(particleCount), () => {
      ...
      const rho_i = max(densitiesBuffer.element(i), float(1e-8)).toVar();
          ...
          Loop(dx.lessThanEqual(1), () => {
            ...
            Loop(j.lessThan(end).and(j.notEqual(i)), () => {
              If(j.notEqual(i), () => {
                ...
                If(r2.lessThan(h2), () => {
                  const invR2 = inverseSqrt(max(r2, float(1e-8).mul(h2)));
                  const invR = float(1.0).div(invR2);
                  const t = float(h).sub(r).toVar();
                  If(t.greaterThan(float(0.0)), () => {
                    const _dir = dir.mul(invR).toVar();
                    const gradW = float(1.0)
                      .mul(float(spiky).mul(t.mul(t)).mul(_dir))
                      .toVar();
                    ...
                    const term = pressure_i
                      ...
                      .toVar();
                    const fi = float(-1.0)
                      .mul(float(mass).mul(float(mass)).mul(term).mul(gradW))
                      .toVar();
